...
*   **算子库：** `operators.h` 文件的43行的 `dragon_op` 枚举了本项目需要用到的**算子**。我们保证operators.h/.cpp中的代码是正确的。
## 提交指南
...
5.  **最终结果：** 提供你用于测试的最终运行命令，并附上一段程序成功运行后截图，以证明你的修复是有效的。
6.  **提示：** **不管你最后是否得到了预期结果**，都应该及时提交解体报告。我们会根据你的阶梯报告对你进行评估。
